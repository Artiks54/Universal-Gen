...
public class RegistryGui implements IGuiHandler {
    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == GUI_COBBLE_GEN) {
            return new CobbleGenBlockContainer(player.inventory, (TileCobbleGen) world.getTileEntity(new BlockPos(x, y, z)), player);
        }
        return null;
    }
    @Override
    ...
    public Object getClientGuiElement ( int ID, EntityPlayer player, World world,int x, int y, int z){
        if (ID == GUI_COBBLE_GEN) {
            return new CobbleGenGui(player.inventory, (TileCobbleGen) world.getTileEntity(new BlockPos(x, y, z)), player);
        }
        return null;
    }
}
